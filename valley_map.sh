...
echo -e "GAP_START_COOR\tGAP_END_COOR\tLINEAR_SIZE\tMAX_COOR\tMAX_COV\tAVG_COV"
...
for i in ${!mapfile[@]}; do
	...
	if   [[ $coverage -ge $mincoverage && ${mapfile[$oldcoor]} -ge $mincoverage ]]; then continue;
	elif [[ $coverage -ge $mincoverage && ${mapfile[$oldcoor]} -lt $mincoverage ]]; then
